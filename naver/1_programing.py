...
예를 들어 고객 한 명당 k = 2장까지 할인 쿠폰을 받을 수 있고, A 고객이 3일 동안 매일 물건을 구매했다면,
이 고객은 할인 쿠폰을 두 장 받습니다.

각 날짜별로 XX 페이를 이용해 물품을 구매한 고객들의 ID가 문자열 형태로 담긴 배열 id_list, 
...
def solution(id_list, k) :
    customer = {}
    for id in id_list :
        id_array = set(id.split())
        for i in id_array :
            if i not in customer.keys() :
                customer[i] = 1 
            elif i in customer.keys() and customer[i] < k :
                customer[i] += 1
    
    result = sum(customer.values())
    return result
