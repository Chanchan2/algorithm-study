...
위 조건에 해당하는 모든 부정행위 의심자의 수험번호를 배열에 담아 사전 순으로 정렬하여 return 하도록 solution 함수를 완성해주세요.
...
def solution(logs) :
    ...
    problems = {}
    for log in logs :
        id, number, score = log.split()
        number = int(number)
        score = int(score)
        if id not in problems.keys() :
            problems[id] = [0] * 101
            solv_cnt[id] = []
        problems[id][number] = score
        if number not in solv_cnt[id] :
            solv_cnt[id].append(number)
    
    print(solv_cnt)
    cheater = [k for k, v in solv_cnt.items() if len(v) >= 5]
    
    result = []
    for i in range(len(cheater) - 1) :
        for j in range(i, len(cheater)) :
            if problems[cheater[i]] == problems[cheater[j]] :
                if cheater[i] not in result :
                    result.append(cheater[i])
                if cheater[j] not in result :
                    result.append(cheater[j])
...
test = ["0001 3 95", "0001 5 90", "0001 5 100", "0002 3 95", "0001 7 80", "0001 8 80", "0001 10 90", "0002 10 90", "0002 7 80", "0002 8 80", "0002 5 100", "0003 99 90"]
test2 = ["1901 1 100", "1901 2 100", "1901 4 100", "1901 7 100", "1901 8 100", "1902 2 100", "1902 1 100", "1902 7 100", "1902 4 100", "1902 8 100", "1903 8 100", "1903 7 100", "1903 4 100", "1903 2 100", "1903 1 100", "1101 1 95", "1101 2 100", "1101 4 100", "1101 7 100", "1101 9 100", "1102 1 95", "1102 2 100", "1102 4 100", "1102 7 100", "1102 9 100"]
test3 = ["1901 10 50", "1909 10 50"]
